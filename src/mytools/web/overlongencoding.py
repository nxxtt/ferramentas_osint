...
def print_results_fn(result: OverlongResult) -> None:
...
class OverlongencodingScanner(BaseScanner):
    """Scanner de Overlong UTF-8 Encoding Bypass — testa bypass de filtros via overlong encoding.."""

    prog = "mytools-overlong"
    description = "Overlong UTF-8 Encoding Bypass — testa bypass de filtros via overlong encoding."
    prompt = "overlong> "
    module_name = "mytools.overlongencoding"
    banner_text = r"""
    ...
    async def run_scan(self, **kwargs):  # type: ignore[override]
        return await scan_overlong_encoding(**kwargs)
...
scanner = OverlongencodingScanner()
main = scanner.main
